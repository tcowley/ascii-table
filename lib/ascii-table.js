...
function normalizeConfig(rows, columns, border, padding, margin) {
    ...
    config.rows = normalizeRows(config.columns, rows, true, true);
    
    return config;
}

function generateTable(config) {
    var margin = stringAlign('', config.margin);
    var padding = stringAlign('', config.padding);
    ...
    config.rows = config.rows.map(function(row, i) {
        ...
        config.columns.forEach(function(column) {
            ...
            var fieldArray;
            fieldArray = stringSplit(row[column.name], width);
            fieldArray = fieldArray.map(function(line, i) {
                if (column.align === 'justify' && i === fieldArray.length - 1) {
                    return stringAlign(line, width, 'left');
                }
                return stringAlign(line, width, column.alignment);
            });
...
function generateEdgeLine(config, isPadding) {
    var separatorLine = stringAlign('', config.margin);
    var separatorBorder;
    var content = ' ';
