...
function asciiTable(rows, columns, border, padding, margin) {
    ...
    config = generateConfig(rows, columns, border, padding, margin);
   
    lines = generateLines(config);
    output = combineLines(config); 
    
    // NOTE: rows may be empty, generate anyway.
...
function generateConfig(rows, columns, border, padding, margin) {
    var config = {};
...
function normalizeColumns(columns, rows, totalWidth, borderWidth, padding, margin) {
    ...
    // - generate an array of column configs, one for every field in the data.
        ...
        // rows is an array of arrays
            ...
            // generate field names that are indexes
        ...
        // if rows is an array of objects: iterate over all, and push all found field names onto array.
    }
...
function normalizePadding(padding) {
    return +padding === padding ? padding : 1;
}
