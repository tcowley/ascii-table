...
function asciiTable(rows, columns, border, padding, margin) {
    var config = normalizeConfig(rows, columns, border, padding, margin);
...
function normalizeConfig(rows, columns, border, padding, margin) {
    ...
    config.width = process.stdout.columns - config.margin;
    config.border = normalizeBorder(border);
    config.borderWidth = config.border.length;
    config.padding = normalizePadding(padding);
    config.columns = normalizeColumns(columns, rows, config.width, config.borderWidth, config.padding);
    config.rows = normalizeRows(columns, rows, true);
...
function generateTable(config) {
    // - format row data
    //   - split all fields to correct width strings
    //   - format all strings
    //   - pad all field arrays with empty strings
    // - combine strings into final table
    //   - iterate over rows and field arrays, joining strings with padding and borders
    // -----------
...
function normalizeRows(columns, rows, skipValidateRows) {
